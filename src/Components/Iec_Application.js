...
import { Row, Col, Button, FormGroup, Input, FormFeedback, Modal, ModalHeader, ModalBody } from 'reactstrap'
...
import { CIN_FORM_MANDATORY_FIELDS, incorporation_partA_data, MOCK_COMPANY_IFNORMATION, MOCK_CORRESPONDENCE_ADDRESS, MOCK_SUBSCRIBER_PARICULAR_DATA, MOCK_DIRECTOR_SUBSCRIBER_DATA, MOCK_INDIVIDUAL_SUBSCRIBER_DATA, MOCK_PARTICULAR_OF_DIRECTORS_DATA, MOCK_PARTICULARS_OF_THE_NAMINE_DATA, MOCK_STAMP_DUTY_DATA, MOCK_COMPANY_STRUCTURE } from './Mock/incorporation_partA_mockdata'
...
const Incorporation_bs = () => {
  ...
  const [isCinFormFilled, setIsCinFormFilled] = useState(false);
  const callCINApiOnConfirmation = async () => {
    ...
    await axios.put(apiUrl, applicationForm, {
      headers: {
  ...
  function getOPCNomieeMock() {
    return {
      opc_din_number: subsriberData?.din,
      opc_nominee_first_name: subsriberData?.person_first_name,
      opc_nominee_middle_name: subsriberData?.person_middle_name,
      opc_nominee_surname: subsriberData?.person_last_name,
      opc_nominee_fathers_first_name: subsriberData?.father_first_name,
      opc_nominee_fathers_middle_name: subsriberData?.father_middle_name,
      opc_nominee_fathers_surname: subsriberData?.father_last_name,
      opc_nominee_gender: subsriberData?.gender,
      opc_nominee_dob: subsriberData?.dob,
      opc_nominee_place_of_birth: subsriberData?.city,
      opc_nominee_PAN_no: subsriberData?.PAN,
      opc_nominee_Aadhar_number: subsriberData?.AADHAR
    }
  }
  const company_structure = MOCK_COMPANY_STRUCTURE;
  const MOCK_CIN_COMPANY_STRUCTURE = {
    'total_authorized_share_capital': company_structure.authorized_capital_registered.value,
    'no_of_shares_authorized_equity': company_structure.no_of_shares.authorized_value,
    'per_share_authorized_equity': company_structure.per_share.authorized_value,
    'total_subscribed_share_capital': company_structure.total_subscribed_capital.value,
    'no_of_shares_subscribed_equity': company_structure.no_of_shares.subscribed_value,
    'per_share_subscribed_equity': company_structure.per_share.subscribed_value
  }

  const MOCK_SUBSRIBER_DIRECTOR_DETAILS = {
    'total_first_susbscriber_valid': '2',
    'no_of_non_individual_valid': '0',
    'no_of_director_cum_subscriber_valid': '2',
    'total_director_cum_subscriver_valid': '2'
  }

  const VALIDATION_OBJ = {
    'New_Company_Info': { screenName: 'COMPANY_INFORMATION', mockData: MOCK_COMPANY_IFNORMATION },
    'company_structure': { screenName: 'COMPANY_STRUCTURE', mockData: MOCK_CIN_COMPANY_STRUCTURE },
    'company_adress': { screenName: 'CORRESPONDENCE_ADDRESS', mockData: MOCK_CORRESPONDENCE_ADDRESS },
    'subscriber_director_company': { screenName: 'SUBSCRIVER_CUM_DIRECTORS', mockData: MOCK_SUBSRIBER_DIRECTOR_DETAILS },
    'subscriber_paricular_non_individual': { screenName: 'SUBSCRIBER_PARTICULAR', mockData: MOCK_SUBSCRIBER_PARICULAR_DATA },
    ...
    'OPC_Nomination': { screenName: 'PARTICULARS_OF_THE_NAMINE', mockData: getOPCNomieeMock() },
    'Stamp_Duty': { screenName: 'STAMP_DUTY', mockData: MOCK_STAMP_DUTY_DATA },
  }

  function isInputMatchedWithMock(inputValue, mockValue) {
    if (inputValue && mockValue && inputValue.trim().toLowerCase() !== mockValue.trim().toLowerCase()) {
      return true
    }
    return false;
  }
  ...
  function matchNameWithMock(inputValue) {
    let nameFound = false;
    let nameArr = [subsriberData?.person_first_name.toLowerCase(),
    subsriberData?.person_middle_name.toLowerCase(),
    subsriberData?.person_last_name.toLowerCase()]
    let inputArr = inputValue.trim().split(' ')
    inputArr.map(i => {
      if (nameArr.indexOf(i.toLowerCase()) > -1) {
        nameFound = true;
      }
    })
    return nameFound;
    ...
    CIN_FORM_MANDATORY_FIELDS[screenName].map(mandField => {
      if (!hasValue(cinForm[mandField])) {
        ...
        if (mandField === 'correspondence_address_line1') {
          ...
          if (mandField.indexOf('dob') !== -1) {
            let dateSplit = cinForm[mandField].split('-');
            let updatedDateFormat = `${dateSplit[2]}-${dateSplit[1]}-${dateSplit[0]}`
            if (isInputMatchedWithMock(updatedDateFormat, mockValue)) {
              errorForm[`${mandField}_error`] = 'Please enter correct value';
            }
          } else {
  ...
  function validateAOCodes() {
    ...
    CIN_FORM_MANDATORY_FIELDS['AO_CODES'].map(mandField => {
      if (!hasValue(cinForm[mandField])) {
        ...
        if (hasValue(mockValue) && mockValue !== 'ND' && isInputMatchedWithMock(cinForm[mandField], mockValue)) {
          errorForm[`${mandField}_error`] = 'Please enter correct value';
        }
      }
    })
  ...
  const navigateToNext = () => {
    ...
    let activeModule = module === 'CIN' ? BSR_CATEGORY_INCORP : module;
    console.log("activeModule...",activeModule)
    let url = `/business-setup-registration/${company}/${activeModule}/${nextScreen} `;
    if (currentScreen === 'Congratulations') {
      callCINApiOnConfirmation()
    } else if (currentScreen === 'New_Company_Info' ||
      currentScreen === 'company_structure'
      || currentScreen === 'subscriber_director_company'
      // || currentScreen === 'subscriber_paricular_non_individual'
      ...
      if (Object.keys(validationStatus).length === 0) {
        navigate(url);
        updateBsrCurrentStatus(url, company, activeModule, nextScreen);
      }
    ...
    } else if (currentScreen === 'PAN_TAN_Information') {
  ...
  const is_aadhar_field = [
    'opc_nominee_Aadhar_number',
  ]
  ...
  const handleValueChange = (e) => {
    ...
    } else if (name === 'opc_nominee_Aadhar_number') {
      matchNumberAndLength(name, value, 12)
    } else if (name === 'opc_nominee_PAN_no') {
      matchNumberAndLength(name, value, 10)
    } else if (name === 'total_authorized_share_capital' || name === 'total_subscribed_share_capital'
      || name === 'no_of_shares_authorized_equity' || name === 'per_share_authorized_equity'
      || name === 'no_of_shares_subscribed_equity' || name === 'per_share_subscribed_equity') {
      matchNumberAndLength(name, value, 7)
    } else {
      setApplicationForm({
        ...applicationForm,
        [name]: value
  ...
  useEffect(() => {
    ...
    if (
      ...
      || currentScreen === 'Particulars_of_directors'
      || currentScreen === 'OPC_Nomination'
      || currentScreen === 'PAN_TAN_Information'
    ...
    if (currentScreen === 'PAN_TAN_Information') {
      let activeCompany = getBSRActiveCompany();
  ...
  return (
    <div>
      ...
      {currentScreen === 'About' && <>
        ...
        <div className="pan_aplictingrid1">
          <div className="pramotors_parta_background">
            <PT_Registration_PartA data={{
              title: "Apply - CIN Registration - Online",
              properties: [
                {
                  name: "Company name",
                  val: activeCompany?.details?.companyname,
                },
              ]
            }} Iecdata={cinparta} />
          </div>
      ...
      {currentScreen === 'New_Company_Info' && <>
      ...
      {currentScreen === 'company_structure' && <>
        <div className="pan_aplictingrid">
          ...
          <div className="partB_panapli">
            <Incorporation_PartB2 renderTextField={renderTextField}
      ...
      {currentScreen === 'subscriber_director_company' && <>
        <div className="pan_aplictingrid">
          ...
          <div className="partB_panapli">
            <Incorporation_PartB4 renderTextField={renderTextField} />
          </div>
        </div>
      </>
      }
      ...
      {currentScreen === 'OPC_Nomination' && <>
        <div className="pan_aplictingrid">
          ...
          <div className="partB_panapli">
            <Incorporation_PartB9 renderTextField={renderTextField} renderSelectField={renderSelectField} />
          </div>
        </div>
      </>
      }
      ...
      {currentScreen === 'Visit_MCA_Portal2' && <>
        <div className="pan_aplictingrid">
          ...
          <div className="partB_panapli">
            <Incorporation_PartB16 data={data} navigateToNext={navigateToNext} />
          </div>
        </div>
      </>
      }
      {currentScreen === 'Upload_E-Form' && <>
        <div className="pan_aplictingrid">
          ...
          <div className="partB_panapli">
            <Incorporation_PartB17 setDisableNext={setDisableNext}/>
          </div>
        </div>
      </>
      }
      ...
      {currentScreen === 'Challan_Payments' && <>
        <div className="pan_aplictingrid">
          ...
          <div className="partB_panapli">
            <Incorporation_PartB21 navigateToNext={navigateToNext}/>
          </div>
        </div>
      </>
      }
      {currentScreen === 'Submit_Payments' && <>
        <div className="pan_aplictingrid">
          ...
          <div className="partB_panapli">
            <Incorporation_PartB22 navigateToNext={navigateToNext}/>
          </div>
        </div>
      </>
      }
          ...
          <Row>
            <Col md={5} className="py-1">
              <div
                ...
                <button className=" py-1 clr_btn_blu rounded-pill"><img src={videoBs} alt="videoBs" width={18} /> &nbsp;Video Tutorials</button>
              </div>
            </Col>
            <Col md={5}>
            </Col>
      ...
      <Modal isOpen={isCinFormFilled} size="l" toggle={() => setIsCinFormFilled(false)} className="modalStyle_Success">
        <ModalHeader toggle={() => setIsCinFormFilled(false)} className="modal_header_success">
          Suceess
        </ModalHeader>
        <ModalBody>
          <div className="modal_body_success"> Successflly Submitted CIN Form. Kindly Proceed further!!</div>
        </ModalBody>
      </Modal>
    </div>
  )
